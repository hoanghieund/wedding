...
const TRAVEL_ICONS: Record<string, WeddingIconName> = {
  "hà nội": "car",
  "bay": "plane",
  "xe máy": "car",
  "xe": "car",
  ...
  return (
    ...
    >
      <div className="space-y-2">
        ...
        <div className="h-px w-24 section-divider" />
      </div>

      {TRAVEL_GUIDANCE.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-[var(--border-soft)] bg-[var(--surface)] px-6 py-12 text-center">
          <p className="font-body-serif text-lg italic text-[var(--text-secondary)]">
            Thông tin di chuyển sẽ được cập nhật sớm.
          </p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="flex items-start gap-4">
                <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-[var(--bg-elevated)] text-2xl border border-[var(--border-soft)]" aria-hidden="true">
                  <WeddingIcon className="h-6 w-6" name={getIcon(item.title)} />
                </span>
